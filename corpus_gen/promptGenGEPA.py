...
Prérequis
- DEEPSEEK_API_KEY dans l'environnement (et éventuellement DEEPSEEK_API_BASE).
- OPENAI_API_KEY si vous revenez à la configuration OpenAI (ancienne, commentée).
- dspy installé.
"""
...
def parse_args(argv: Optional[List[str]] = None) -> Config:
    ...
    parser.add_argument("--deepseek-api-base", type=str, default=DEFAULT_CFG.deepseek_api_base, help="Endpoint DeepSeek (ex: https://api.deepseek.com).")
    ...
    return Config(
        ...
        deepseek_api_base=args.deepseek_api_base,
...
def ensure_deepseek_api_key() -> str:
    ...
    if not api_key:
        raise RuntimeError("Définissez la variable d'environnement DEEPSEEK_API_KEY")
    return api_key


def configure_generator_lm(model: str, temperature: float, api_base: str) -> dspy.LM:
    # Ancienne config OpenAI (conservée) :
...
def configure_comparator_lm(model: str, api_base: str) -> dspy.LM:
    # Ancienne config OpenAI (conservée) :
    # api_key = ensure_api_key()
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    configure_generator_lm(cfg.generator_model, cfg.reflection_temperature, cfg.deepseek_api_base)
    lm_comparator = configure_comparator_lm(cfg.comparator_model, cfg.deepseek_api_base)
