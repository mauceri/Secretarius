...
export async function fetchWikiOpJson(
  api: any, op: string, arg: string, exec: Exec = execWikiSandbox,
): Promise<{ ok: boolean; text: string }> {
  const argv = ["python3", "/wiki-tools/wiki.py", op];
  if (arg) argv.push(arg);
  const { code, stdout, stderr } = await exec(api, argv);
  if (code !== 0) return { ok: false, text: `Erreur wiki : ${(stderr || stdout || "échec").slice(0, 500)}` };
  const lines = stdout.split("\n").map((l) => l.trim()).filter(Boolean);
  const lastLine = lines.length ? lines[lines.length - 1] : "";
  let json: any;
  try {
...
export function formatWikiResult(op: string, json: any, regime: WikiOpRegime = "brief"): string {
  ...
  switch (op) {
    case "query": {
      ...
      if (!brief && !path) return "Réponse wiki vide ou inattendue.";
      ...
      if (path) parts.push(escapeHtml(path));
      return parts.join("\n\n");
    }
    case "capture": {
    ...
    case "delete": {
      ...
      if (!affected.length) return "Aucune page affectée.";
