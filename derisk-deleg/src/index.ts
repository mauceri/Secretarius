...
async function runAndRead(
  ...
  for (const m of msgs) {
    ...
    const t = textOf(m);
    if (role === "assistant" && t.trim()) answer = t;
  }
...
function delegateGog(api: any, op: string, arg: string): Promise<string> {
...
export default definePluginEntry({
  ...
  description:
    "Deterministic command tools delegating wiki ops (capture/status) to the wiki subagent.",
  register(api: any) {
    api.registerTool({
      name: "wiki_capture",
      description:
        "Capture a URL/note into the wiki inbox (delegates 'op: capture' to the wiki agent).",
      parameters: Type.Object({
        command: Type.Optional(
          Type.String({ description: "Raw args: #tags + URL or text to capture." }),
        ),
      }),
    ...
    api.registerTool({
      name: "wiki_status",
      description:
        "Report wiki ingestion status (delegates 'op: status' to the wiki agent).",
      parameters: Type.Object({
    ...
    api.registerTool({
      name: "wiki_ingest",
      description:
        "Process the wiki capture queue (delegates 'op: ingest' to the wiki agent; runs async).",
      parameters: Type.Object({
        command: Type.Optional(Type.String({ description: "Unused." })),
      }),
      async execute(_id: string, _params: { command?: string }) {
    ...
    api.registerTool({
      name: "wiki_verify",
      description:
        "Marque une page comme vérifiée (prépare l'action ; confirmation requise via /confirm, jamais direct).",
      parameters: Type.Object({
        command: Type.Optional(Type.String({ description: "Le slug de la page à marquer vérifiée." })),
      }),
      async execute(_id: string, params: { command?: string }) {
        const slug = (params?.command ?? "").trim();
        if (!slug) return { content: [{ type: "text", text: "Usage: /verifie <page>" }] };
    ...
    api.registerTool({
      name: "wiki_delete",
      description:
        "Supprime une page du wiki et sa cascade (toujours un essai à blanc puis /confirm, jamais direct).",
      parameters: Type.Object({
        command: Type.Optional(Type.String({ description: "Le slug de la page à supprimer." })),
      }),
      async execute(_id: string, params: { command?: string }) {
        const slug = (params?.command ?? "").trim();
        if (!slug) return { content: [{ type: "text", text: "Usage: /supprimer <page>" }] };
    ...
    api.registerTool({
      name: "source_read",
      description:
        "Read/summarize an external web page NOW via the anti-injection scout agent (delegates 'url: <url>').",
      parameters: Type.Object({
        command: Type.Optional(
          Type.String({ description: "Raw args: the URL to read." }),
        ),
      }),
      async execute(_id: string, params: { command?: string }) {
    ...
    api.registerTool({
      name: "gog_inbox",
      description:
        "List recent inbox emails (delegates 'op: inbox' to the gog agent, which runs gog in its sandbox).",
      parameters: Type.Object({
        command: Type.Optional(
          Type.String({ description: "Optional Gmail query (default: in:inbox)." }),
        ),
      }),
      async execute(_id: string, params: { command?: string }) {
    ...
    api.registerTool({
      name: "gog_send",
      description:
        "Prépare un brouillon d'email pour confirmation (n'envoie PAS). Après l'appel, indiquer à l'utilisateur de taper /confirm pour envoyer.",
      parameters: Type.Object({
    ...
    api.on("before_agent_reply", async (event: any, ctx: any) => {
      ...
      if (cmd === "/maj") {
        ...
        const report: string[] = [];
        let deferredUnit: string | null = null;
        ...
        if (deferredUnit) {
          ...
          try {
            execSync(
              `systemd-run --user --on-active=5s --unit=maj-restart-${Date.now()} systemctl --user restart ${deferredUnit}`,
            );
          } catch (e: any) {
    ...
    api.on(
      ...
      async (event: any, ctx?: any) => {
        ...
        if (!/\bgog\b/.test(cmd)) return;
        const isSensitive =
          /\b(send|reply|forward|delete|trash|remove|share|revoke|rm)\b/i.test(cmd);
        if (!isSensitive) return;
        const agentId =
          ctx?.agentId ?? event?.agentId ?? event?.ctx?.agentId;
        if (agentId === "gog") return; // flux confirmé (/confirm -> agent gog) : autorisé
        return {
          block: true,
          blockReason:
