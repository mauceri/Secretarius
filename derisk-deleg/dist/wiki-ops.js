...
export async function execWikiSandbox(api, argv) {
    try {
...
export async function runWikiOp(api, op, arg, exec = execWikiSandbox, regime = "brief") {
    ...
    if (code !== 0)
        return `Erreur wiki : ${(stderr || stdout || "échec").slice(0, 500)}`;
...
export function formatWikiResult(op, json, regime = "brief") {
    ...
    switch (op) {
        case "query": {
            ...
            if (uri)
                parts.push(`<a href="${uri}">Voir la réponse complète</a>`);
            return parts.join("\n\n");
        }
        case "capture": {
