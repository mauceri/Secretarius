...
def build_text_dataset(ds: DatasetDict, system_prompt: str, user_prefix: str, num_proc: int) -> DatasetDict:
...
def parse_args():
    ...
    p.add_argument("--packing", action="store_true", help="Active le constant-length packing")
...
def main():
    ...
    if have_cuda:
        ...
        print("********************************************Aucun accelerator disponible")
    # 1) Tokenizer
    ...
    ds_tok = {k: _map_dataset(ds_txt[k], tok_and_mask, batched=True, remove_columns=remove_cols, num_proc=args.num_proc)
              for k in ds_txt.keys()}
    ds_tok = DatasetDict(ds_tok)
    ds_tok = DatasetDict({k: ds_tok[k].filter(lambda x: x["has_label"]).remove_columns(["has_label"])
    ...
    if args.device == "cpu":
        ...
        if not have_cuda:
            print("Device CUDA demandé mais indisponible, bascule en CPU.")
            device_map = "cpu"
            have_cuda = False
        else:
            device_map = "cuda"
    ...
    # Compat Transformers: evaluation_strategy (new) vs eval_strategy (old)
    ...
    try:
        ...
        targs_kwargs.pop("evaluation_strategy", None)
