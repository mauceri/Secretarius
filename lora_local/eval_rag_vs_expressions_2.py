#!/usr/bin/env python3
"""
Expérience 2 — RAG classique vs indexation par expressions.

Même protocole que l'expérience 1, mais les requêtes sont générées
à partir des EXPRESSIONS du document (pas du texte brut).

Cela lève le biais de l'expérience 1 qui favorisait le RAG classique
(requêtes issues du texte brut → alignement naturel avec l'embedding du chunk).

Quatre conditions identiques :
  ...
  D) Late interaction symét.    : score = Σ_i max_j cos(expr_query_i, expr_doc_j)
                                  (vraie late interaction — expressions extraites côté requête par LoRA)

Protocole :
  - Corpus  : corpus_synth_indexed_1000.jsonl (chunks + expressions_caracteristiques)
  - Requête : question synthétique générée par DeepSeek à partir des EXPRESSIONS du chunk
...
Usage :
    ...
    python eval_rag_vs_expressions_2.py [--n 200] [--out eval_rag2_results.json]
"""
...
QUERY_GEN_SYSTEM = (
    "Voici une liste d'expressions-clés extraites d'un document en français. "
    "Écris une question naturelle et précise qu'un utilisateur pourrait taper "
    "dans un moteur de recherche pour trouver un document contenant ces informations. "
    "Réponds UNIQUEMENT par la question, sans explication ni ponctuation finale."
)
...
def load_or_generate_queries(chunks: list[dict], api_key: str, cache: Path) -> list[str | None]:
    ...
    for i, chunk in enumerate(chunks):
        ...
        q = _deepseek_call(QUERY_GEN_SYSTEM, ", ".join(chunk["expressions"]), api_key)
        queries.append(q)
        time.sleep(0.08)
    print()
...
def load_or_extract_query_expressions(
    queries: list[str | None], cache: Path
) -> list[list[str]]:
    """Extrait les expressions de chaque requête via LoRA. Cache les résultats."""
    if cache.exists():
        cached = json.loads(cache.read_text(encoding="utf-8"))
    ...
    for i, q in enumerate(queries):
        ...
        if q:
            exprs_list.append(lora_extract(q))
        else:
            exprs_list.append([])
    print()
...
def main() -> None:
    ...
    parser.add_argument("--queries-cache",        default="eval_rag2_queries.json")
    parser.add_argument("--query-exprs-cache",    default="eval_rag2_query_exprs.json")
    parser.add_argument("--out",                  default="eval_rag2_results.json")
    ...
    all_query_exprs = load_or_extract_query_expressions(queries, Path(args.query_exprs_cache))
