...
**Deuxième correction (2026-08-17, citation directe du papier fournie par
l'utilisateur)** : ce document a déjà été corrigé une fois sur ce point (voir
historique), et cette citation directe permet enfin de trancher plus
proprement que mes reconstructions précédentes :

> « However, attention scores remain vulnerable to ISA, as the attention
> scores generated by attention queries and keys are not obfuscated by the
> key matrices. Consequently, in AloePri, permutations of attention heads
> and blocks are introduced to further protect attention scores. »

Sans ambiguïté : les scores d'attention (produits scalaires Q·K) **ne sont
pas du tout touchés** par les protections de l'embedding (bruit, matrices
clés) — pas « partiellement atténués », comme le suggérait une hypothèse
intermédiaire de ce document (retirée). Le 87,14 % de TTRSR au Tableau 4
mesure le taux de récupération de tokens obtenu **en exploitant ce canal
précis**, qui reste grand ouvert tant que l'attention elle-même n'est pas
permutée.

**Conséquence** : puisque ce canal ne dépend pas de la permutation secrète de
l'embedding, un attaquant peut plausiblement entraîner un modèle d'inversion
sur un **modèle public de référence** (mêmes scores d'attention pour un texte
connu) et l'appliquer directement aux scores observés sur le déploiement
obfusqué, **sans jamais avoir besoin de connaître ni de casser la
permutation**. Contrairement à ce qu'affirmait la version précédente de ce
document, **la rotation de la permutation/des clés d'embedding ne protège
donc probablement pas contre ce canal spécifique** — elle n'a simplement
aucune prise dessus. Ce point redevient la conclusion retenue, cette fois
ancrée sur une phrase directe du papier plutôt que sur une reconstruction
architecturale.
