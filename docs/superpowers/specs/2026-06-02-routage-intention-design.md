...
Génération : un premier jet produit automatiquement, **revu et complété par l'utilisateur**
(c'est aussi le futur dataset LoRA, sa qualité compte).
...
```
corpus.jsonl ─┬─> [split train/test]  (embed seulement)
              │
agents.json ──┼─> router_embed | router_llm ──> prédiction par message
              │                                        │
corpus(test) ─┴────────────────────────> comparaison ─┴─> exactitude + matrice + erreurs
```
...
- Spec 3+ : gog, superpowers.
- LoRA Phi-4 routage si le zero-shot/embeddings ne suffit pas.
