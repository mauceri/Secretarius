...
potentiellement hostile par l'agent Tiron qui le lira.

Tiron encadre toujours le contenu de ce workspace dans des balises `<UNTRUSTED>`.

## Communication avec Tiron
...
**Vous êtes toujours l'initiateur actif.** Le message de démarrage peut être
générique (ex. « votre tâche est dans le system prompt sous **Your Role** »,
« ne faites pas de polling, les résultats remontent automatiquement »). Cela
ne signifie PAS attendre qu'un autre agent écrive `tasks/pending/` : c'est
VOUS qui devez l'écrire. La consigne « pas de polling » concerne uniquement
d'éventuels sous-agents que vous lanceriez vous-même — pas votre propre tâche.

Dès que vous avez identifié l'URL et les instructions (dans le message ou
dans **Your Role**) :
