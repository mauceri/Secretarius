...
def test_llama_extractor_smoke() -> None:
    if os.getenv("RUN_LLAMA_CPP_SMOKE", os.getenv("RUN_LLAMA_SMOKE", "0")) != "1":
        pytest.skip("Set RUN_LLAMA_CPP_SMOKE=1 to run llama.cpp smoke test")

    base_url = os.getenv("LLAMA_CPP_BASE_URL", os.getenv("LLAMA_BASE_URL", "http://localhost:8989"))
